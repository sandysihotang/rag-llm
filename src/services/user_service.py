...
class UserService():
    ...
    def register_user(self, email: str, password: str, session: Session):
        ...
        try:
            user_check = UsersRepository.get_data_email(session, email=email)
            if (user_check != None):
                raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Email Already Exist")
            new_id = UsersRepository.insert_new_user(session, user)
            user.id = new_id
            session.commit()
            return user
        except Exception as e:
            ...
            session.rollback()
            raise e
    def get_user(self, email:str, password: str, session: Session):
        try:
            user = UsersRepository.get_data_email(session, email=email)
            if(user.id == 0):
                return False, 0
            fernet = Fernet(self.key_password)
